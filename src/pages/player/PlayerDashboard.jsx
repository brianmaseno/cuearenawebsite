...
const PlayerDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [ongoingResult, notifResult, achievementsResult] =
      ...
      const ongoingData =
        ongoingResult.status === "fulfilled" ? ongoingResult.value.data : {};
      const notifData =
      ...
      if (ongoingResult.status === "rejected")
        console.error(
          "Failed to fetch ongoing activities:",
          ongoingResult.reason?.message,
        );
      ...
      if (achievementsResult.status === "rejected")
        console.error(
          ...
          achievementsResult.reason?.message,
        );
  ...
  const handleInvitationResponse = async (
    ...
    try {
      ...
      if (invitationId) {
        ...
        await api.post(`/invitations/respond-by-target/${type}/${targetId}`, {
          status,
        });
      }
          ...
          (t) => (
            <div className="flex flex-col gap-2">
              ...
              <span className="text-xs opacity-90">
                You need more funds in your wallet to accept this invitation.
              </span>
              <button
                onClick={() => {
                  toast.dismiss(t.id);
  ...
  return (
    <DashboardLayout title="Active Activities">
      <div
        className="space-y-10 pb-20 max-w-[1600px] mx-auto px-4 md:px-8 bg-background min-h-screen"
      ...
      >
        ...
        <div className="pt-8">
          ...
          <p className="text-text/60 font-medium">
            Manage your ongoing matches, battles, and tournaments.
          </p>
        </div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar">
              {data.achievements.map((ua) => {
                ...
                return (
                  <div
                    key={ua.id}
                    className="min-w-[200px] bg-base2/20 p-4 rounded-2xl border border-base2/10 relative group shrink-0 overflow-hidden"
                  >
                    ...
                    <div className="relative z-10 flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-xs font-black text-text-emphasis uppercase truncate tracking-tight">
                          {ach?.name}
                        </h4>
          ...
          {[
            {
              ...
              label: "Matches",
              count:
                data.matches.length + (data.tournamentMatches?.length || 0),
              icon: Target,
              color: "blue",
            },
            {
            ...
            const activeColorClass =
              tab.color === "blue"
                ? "bg-blue-500 shadow-[0_10px_25px_-5px_rgba(59,130,246,0.4)]"
                : tab.color === "emerald"
                  ? "bg-emerald-500 shadow-[0_10px_25px_-5px_rgba(16,185,129,0.4)]"
        ...
        {activeTab === "matches" ? (
          <div className="grid-dashboard">
            ...
            data.tournamentMatches.length === 0 ? (
              ...
              [
                ...
                const activeSetRes = match.setsResults?.find(
                  (s) => s.setIndex === currentActiveSet,
                );

                return (
                  <AuraCard
                    key={match.id}
                    className="p-0 rounded-2xl overflow-hidden group hover:ring-2 ring-primary/20 transition-all border-none perspective-1000"
                  >
                    <div className="bg-base2/10 px-4 py-2.5 flex flex-wrap items-center justify-between gap-2 border-b border-base2 preserve-3d">
                      <div className="flex items-center gap-2.5 min-w-0 flex-1">
                        ...
                        <h3 className="text-[13px] font-black uppercase tracking-tighter text-text-emphasis truncate">
                          {match.type === "tournament"
                            ? match.tournament?.name || match.tournamentId?.name
                            : "Exhibition Match"}
                        </h3>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center justify-end gap-2 shrink-0 ml-auto">
                        {(match.stakeAmount > 0 ||
                          (match.type === "tournament" &&
                            (match.tournament?.stakePerPlayer ||
                          ...
                          <div className="flex flex-wrap items-center justify-end gap-1.5">
                            <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100 flex items-center gap-1">
                              ...
                              {(match.type === "tournament"
                                ? match.tournament?.stakePerPlayer ||
                                  match.tournamentId?.stakePerPlayer ||
                                  0
                                : match.stakeAmount || 0
                              ).toLocaleString()}
                            </span>
                              ...
                              {match.type === "tournament"
                                ? (
                                    (match.tournament?.stakePerPlayer ||
                                      match.tournamentId?.stakePerPlayer ||
                                      0) *
                                    (match.tournament?.maxPlayers ||
                                      match.tournamentId?.maxPlayers ||
                                      0) *
                                    0.85
                                  ).toLocaleString()
                                : (
                      ...
                      <div className="flex items-start justify-between gap-3 relative min-w-0">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col items-center gap-2">
                          <div className="relative">
                            <img
                              src={
                                match.player1?.profilePhoto ||
                                match.player1Id?.profilePhoto ||
                                `https://ui-avatars.com/api/?name=${match.player1?.fullName || match.player1Id?.fullName}&background=random`
                              }
                              alt={
                              ...
                              className={`w-14 h-14 rounded-2xl object-cover ring-2 shadow-md transition-all ${
                                ...
                                (
                                  ...
                                  ? "ring-green scale-105"
                                  : (
                                        activeSetRes?.winnerId?.id ||
                                        activeSetRes?.winnerId ||
                                        ""
                                      ).toString() ===
                                      (
                                        match.player1?.id ||
                                    ...
                                    ? "ring-primary border-4 border-primary/20"
                                    : "ring-base3"
                              }`}
                            />
                          </div>
                          <div className="text-center">
                            <p className="text-sm font-bold truncate max-w-[110px] sm:max-w-[140px] text-text-emphasis">
                              {match.player1?.fullName ||
                                match.player1Id?.fullName}
                            </p>
                            <div className="mt-1 flex flex-col items-center gap-0.5">
                              <span className="text-xs font-black text-primary">
                                {match.setsResults?.filter(
                                  (s) =>
                                    (
                                      s.winnerId?.id ||
                                      s.winnerId ||
                              ...
                              <span
                                className={`text-xs font-black uppercase tracking-wider ${match.player1Accepted ? "text-green" : "text-orange"}`}
                              >
                        ...
                        <div className="w-28 sm:w-36 md:w-40 shrink-0 flex flex-col items-center gap-2">
                          <div className="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-primary/80 text-center max-w-full leading-tight mb-2 drop-shadow-sm break-words">
                            {match.type === "tournament"
                              ? `Round ${match.round}`
                              : match.title || "Direct"}
                          </div>
                          <div className="text-xl font-black text-primary/10 italic">
                            VS
                          </div>
                          <div className="flex flex-col items-center gap-1">
                            <p className="text-xs font-bold text-text/60">
                              {match.type === "tournament"
                                ? "Tournament Match"
                                : `Org: ${match.organizer?.fullName || match.organizerId?.fullName}`}
                            </p>
                            <p className="text-xs font-bold text-text/60">
                              {match.location ||
                                match.venue ||
                        ...
                        <div className="flex-1 min-w-0 flex flex-col items-center gap-2">
                          <div className="relative">
                            <img
                              src={
                                match.player2?.profilePhoto ||
                                match.player2Id?.profilePhoto ||
                                `https://ui-avatars.com/api/?name=${match.player2?.fullName || match.player2Id?.fullName}&background=random`
                              }
                              alt={
                              ...
                              className={`w-14 h-14 rounded-2xl object-cover ring-2 shadow-md transition-all ${
                                ...
                                (
                                  ...
                                  : (
                                    ...
                                    ? "ring-violet border-4 border-violet/20"
                                    : "ring-base3"
                              }`}
                            />
                          </div>
                          <div className="text-center">
                            ...
                            <div className="mt-1 flex flex-col items-center gap-0.5">
                              ...
                              <span
                                className={`text-xs font-black uppercase tracking-wider ${match.player2Accepted ? "text-green" : "text-orange"}`}
                              >
                      ...
                      {match.tableSession && (
                        <div className="mt-4 p-3 bg-primary/5 border border-primary/10 rounded-xl flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="text-[10px] font-black text-text/40 uppercase leading-none mb-1">
                                Assigned Table
                              </p>
                              <p className="text-xs font-black text-text-emphasis">
                                TABLE{" "}
                                {match.tableSession.table?.tableNumber ||
                                  match.tableSession.tableId}
                              </p>
                            </div>
                          </div>
                          <div className="text-right">
                            ...
                            <span
                              className={`text-[10px] font-black uppercase ${match.tableSession.status === "active" ? "text-emerald-500" : "text-primary"}`}
                            >
                    ...
                    <div className="bg-gradient-to-b from-base2/30 to-transparent p-4 border-t border-base2 mt-auto space-y-4 relative z-20">
                      {match.myStatus === "pending" ? (
                        <div className="flex gap-2">
                          <button
                            ...
                            className="relative z-30 pointer-events-auto flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold border border-red/20 text-red hover:bg-red/5 transition-all"
                          >
                            <XCircle size={16} />
                            Decline
                          </button>
                          <button
                            ...
                            className="relative z-30 pointer-events-auto flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-primary text-base3 text-sm font-bold hover:shadow-lg hover:shadow-primary/20 transition-all"
                          >
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex flex-wrap gap-2 justify-center">
                            {Array.from({ length: match.setsCount || 1 }).map(
                              (_, idx) => {
                                const sRes = match.setsResults?.find(
                                  (s) => s.setIndex === idx,
                                );
                                const isActive = currentActiveSet === idx;
                                const isP1 =
                                  userId ===
                                  (match.player1Id?.id || "").toString();
                                const isP2 =
                                  userId ===
                                  (match.player2Id?.id || "").toString();
                                  ...
                                  <div
                                    ...
                                    className={`flex-1 min-w-[50px] py-1.5 rounded-lg text-xs font-black uppercase transition-all border flex flex-col items-center justify-center ${
                                      isActive
                                        ...
                                        : sRes
                                          ? "bg-base3/80 border-base2/50 opacity-90 border-primary/30"
                                          : "bg-base2/10 border-transparent text-text/20"
                                    }`}
                                  >
                                    <span
                                      className={
                          ...
                          {match.status === "completed" ? (
                            (() => {
                              ...
                              if (match.type === "tournament") {
                                ...
                                if (!isEliminated) {
                                  if (myWonMatch) {
                                    ...
                                    const nextMatchDate =
                                      match.loserNextMatchId?.scheduledAt;
                                    return (
                                      <div className="w-full py-3 rounded-xl text-sm font-black flex flex-col items-center justify-center gap-1 shadow-sm border bg-orange/5 text-orange border-orange/20">
                                        ...
                                        <span className="text-[10px] opacity-75 uppercase tracking-widest font-bold">
                                          Your next match is{" "}
                                          {nextMatchDate
                                            ? `on ${new Date(nextMatchDate).toLocaleDateString()}`
                                            : "on the way"}
                                        </span>
                                      </div>
                                    );
                              ...
                              return (
                                <div
                                  className={`w-full py-3 rounded-xl text-sm font-black flex flex-col items-center justify-center gap-1 shadow-sm border ${
                                    myWonMatch
                            ...
                            match.type === "tournament" ? (
                              <Link
                                ...
                                className="relative z-30 pointer-events-auto w-full bg-primary text-base3 py-3 rounded-xl text-sm font-black flex items-center justify-center gap-2 hover:scale-[1.02] transition-all shadow-md shadow-primary/20"
                              >
                                <Trophy size={16} />
                                ENTER ROOM
                              </Link>
          ...
          <div className="grid-dashboard">
            {data.battles.length === 0 ? (
              ...
              data.battles.map((battle) => (
                <AuraCard
                  key={battle.id}
                  className="p-0 rounded-2xl overflow-hidden flex flex-col group border-none shadow-sm transition-all hover:shadow-md perspective-1000"
                >
                  <div className="bg-base2/10 p-4 flex justify-between items-center border-b border-base2 preserve-3d">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black uppercase text-text/40 tracking-widest">
                        Multiplayer Battle
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    <h3 className="text-base font-bold text-text-emphasis mb-1 truncate">
                      {battle.title}
                    </h3>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      <div className="bg-emerald-50 border border-emerald-100 p-2 rounded-xl text-center">
                        ...
                        <p className="text-xs font-black text-emerald-600 leading-none">
                          KES {battle.stakeAmount.toLocaleString()}
                        </p>
                      </div>
                      <div className="bg-primary/5 border border-primary/10 p-2 rounded-xl text-center">
                        <p className="text-[9px] font-black uppercase text-primary/60 leading-none mb-1">
                          TOTAL POT
                        </p>
                            ...
                            <div className="flex items-center gap-2 overflow-hidden">
                              <img
                                src={
                                  ...
                                  `https://ui-avatars.com/api/?name=${p.userId?.fullName}&background=random`
                                }
                                className="w-6 h-6 rounded-lg object-cover ring-1 ring-base2 shadow-sm"
                                alt=""
                              />
                              <span
                                className={`text-[11px] font-bold truncate ${isMe ? "text-primary" : isWinner ? "text-green" : "text-text-emphasis"}`}
                              >
                              ...
                              {isWinner && (
                                <TrophyIcon
                                  size={10}
                                  className="text-green shrink-0 animate-bounce"
                                />
                              )}
                            </div>
                            <span
                              className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded border ${
                                p.status === "accepted"
                                  ...
                                  : p.status === "declined"
                                    ? "bg-red/5 text-red/60 border-red/10"
                                    : "bg-yellow/5 text-yellow border-yellow/10"
                              }`}
                            >
                              {p.status}
                            </span>
                    ...
                    {battle.myStatus === "pending" ? (
                      ...
                      <div className="w-full">
                        ...
                        {battle.status === "completed" && (
                          <div
                            className={`w-full py-2.5 rounded-xl text-[10px] font-black flex flex-col items-center justify-center gap-1 border ${
          ...
          <div className="grid-dashboard">
            {data.tournaments.length === 0 ? (
              ...
              data.tournaments.map((t) => (
                <AuraCard
                  key={t.id}
                  className="p-0 rounded-2xl overflow-hidden flex flex-col group border-none shadow-sm transition-all hover:shadow-md h-full relative perspective-1000"
                >
                  ...
                  <div className="p-5 flex-1 preserve-3d">
                    <h3 className="text-base font-bold text-text-emphasis mb-2 truncate">
                      {t.name}
                    </h3>
                  ...
                  <div className="p-3 border-t border-base2 mt-auto preserve-3d">
                    {t.myStatus === "pending" ? (
                      ...
                      <Link
                        ...
                        className="relative z-30 pointer-events-auto w-full bg-blue text-base3 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 shadow-sm hover:scale-[1.02] transition-all hover:bg-blue/90"
                      >
                        <TrophyIcon size={14} />
                      ...
                      <Link
                        to={`/dashboard/tournament/${t.id}`}
                        className="relative z-30 pointer-events-auto w-full bg-primary text-base3 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 shadow-md shadow-primary/20 hover:scale-[1.02] transition-all"
                      >
                        Enter Room
