          ...
          >
            ...
            {players.map((player, index) => {
              ...
              return (
                ...
                >
                  <div className={`group relative backdrop-blur-xl border rounded-2xl p-2.5 md:px-6 md:py-3.5 transition-all duration-400 flex md:grid md:grid-cols-12 md:items-center gap-3 md:gap-4 overflow-hidden ${isMe
                    ...
                    {index < 3 && (
                      <motion.div
                        className={`absolute -inset-10 opacity-[0.08] blur-[80px] pointer-events-none z-0 ${index === 0 ? 'bg-primary' : index === 1 ? 'bg-text' : 'bg-orange'
                          }`}
                    ...
                    <div className="md:col-span-7 flex items-center gap-3 md:gap-4 relative z-10 flex-1 min-w-0">
                      {/* Rank Indicator */}
                      <div className={`shrink-0 w-8 h-8 md:w-9 md:h-9 rounded-xl flex items-center justify-center shadow-lg border transition-transform group-hover:scale-105 duration-400 ${index === 0 ? 'bg-primary text-base3 border-primary/50 shadow-primary/20' :
                      ...
                      <div className="relative shrink-0">
                        <div className={`w-8 h-8 md:w-10 md:h-10 rounded-xl border overflow-hidden bg-base3 p-0.5 transition-all duration-400 ${index === 0 ? 'border-primary shadow-sm scale-105' : isMe ? 'border-primary/40' : 'border-base2 group-hover:border-primary/20'
                          }`}>
                          {player.profilePhoto ? (
                      ...
                      <div className="min-w-0">
                        <h3 className={`font-black tracking-tight transition-colors duration-400 truncate ${index === 0 ? 'text-sm md:text-base text-primary' : isMe ? 'text-xs md:text-sm text-primary' : 'text-xs md:text-sm text-text-emphasis group-hover:text-primary'
                          }`}>
                          {player.fullName}
                        </h3>
                    ...
                    <div className="md:col-span-5 flex items-center justify-end gap-3 md:gap-10 relative z-10 shrink-0">
                      <div className="flex flex-col md:items-center">
                        ...
                        <div className={`font-black tabular-nums transition-all duration-400 ${index === 0 ? 'text-base md:text-xl text-primary' : isMe ? 'text-sm md:text-base text-primary' : 'text-sm md:text-base text-text-emphasis group-hover:text-primary'
                          }`}>
                          {player.points || 0}
                        </div>
                      </div>

                      <div className={`flex items-center gap-1.5 md:gap-2 px-2 md:px-3 py-1 md:py-1.5 rounded-lg md:rounded-xl border transition-all duration-400 ${isMe ? 'bg-primary/10 border-primary/20' : index === 0 ? 'bg-primary/5 border-primary/10' : 'bg-base2/5 border-transparent group-hover:border-base2/40'
                        }`}>
                        ...
                        <span className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest ${player.rankTrend === 'up' ? 'text-green' :
